...
To access the full range of WEP features, sign up for an account. Creating an account lets you save the results of analyses to view them at a later date and to create custom farm, catchment and cost profiles to reuse across analysis modes.
...
Farm profiles describe the demand placed on dam water. You can configure farm profiles to capture livestock drinking requirements, spray requirements, irrigation water requirements and other uses. 
...
To create a farm profile, go to *Settings* in the navbar and select *Farm profiles*. You then need to give the farm profile a name and then fill out the web form describing the demand placed on the dam water. Click on each of the sections to bring up the form widgets. Here, we only have 800 sheep using the dam for drinking in January, February and March so we add that to the form. WEP will use daily temperature data to compute a drinking rate for these sheep for each day that a dam simulation runs for. 

```{=html}
<div class="video-container">
    <iframe class="responsive-iframe" src="https://drive.google.com/file/d/16uxrC8xv8UmGZJj7Zo8Z1-prtBuI9Jl0/preview" allow="autoplay"></iframe>
</div>
```

Head to the [user guide](user-guide/profiles.qmd#farm-profiles) to find more details on configuring profiles.
...
```{=html}
<div class="video-container">
    <iframe class="responsive-iframe" src="https://drive.google.com/file/d/16pwesMPt4OGCOVzCI8XKA39gcld1rmVs/preview" allow="autoplay"></iframe>
</div>
```
...
![Access the cost profile form via the Settings menu](img/getting-started/settings-menu-cost-profile.jpg)

![Fill out the cost profile form and save the profile to your account](img/getting-started/cost-profile-form.jpg)
...
* **Farm profile:** Select a farm profile; this should be the one we created earlier which was called `Getting started demo`.
...
* **Dam morphometry:** Use the sliders to specify the dam morphometry to set the surface area and volume of the dam. This dam has a surface length of `55 m`, a depth of `5 m`, and slope of `33 %`. 
...
![Use the form widgets on the sidebar to configure your simulation. Click on the arrow for each section to bring up the widgets to enter values](img/getting-started/form-sidebar.jpg)
...
Under the *Profiles* dropdown list you can bring up summaries of farm, catchment and cost profiles saved to your account. 

![Use the dropdown list to see summaries of farm, catchment and cost profiles saved to your account](img/getting-started/accounts-analysis-modes.jpg)